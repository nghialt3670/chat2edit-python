...
from chat2edit.models import ChatCycle, Context, Message

OTC_PROMPT_TEMPLATE = """
Given this execution context:
...
class OtcStrategy(PromptStrategy):
    ...
    def _create_otc_sequence(self, chat_cycle: ChatCycle) -> str:
        ...
        for edit_cycle in chat_cycle.edit_cycles:
            if edit_cycle.prompting_result.error or edit_cycle.execution_result.error:
                continue

            answer = edit_cycle.prompting_result.messages[-1]
            thinking, _ = self._extract_thinking_commands(answer)
            executed_blocks = edit_cycle.execution_result.blocks
            executed_cmds = self._create_commmands_str(executed_blocks)

            steps.append(thinking)
            steps.append(executed_cmds)

            feedback = edit_cycle.execution_result.feedback
    ...
    def _create_obs_from_request(self, message: Message) -> str:
